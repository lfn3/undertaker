...
(defn push-interval* [state interval-name]
  (let [id (inc (::interval-id-counter state))]
    (-> state
        ...
        (update ::interval-stack conj [interval-name id (::bytes-counter state)]))))

(defn pop-interval* [state interval-id]
  (let [interval-to-update (last (::interval-stack state))]
    ...
    (-> state
        ...
        (update ::completed-intervals conj (conj interval-to-update (::bytes-counter state))))))
...
(defrecord WrappedRandomSource
  ...
  (pop-interval [_ interval-id]
    (swap! state-atom pop-interval* interval-id))
